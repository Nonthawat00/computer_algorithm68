...
if 'students' not in st.session_state:
    st.session_state.students = [
        {"id": "001", "name": "นายสมชาย ใจดี", "score": 100},
        {"id": "002", "name": "นางสาวสมหญิง รักเรียน", "score": 100},
        {"id": "003", "name": "นายกอไก่ ขยันมาก", "score": 100},
    ]
...
# --- TAB 3: จัดการรายชื่อ ---
with tab3:
    st.subheader("➕ เพิ่ม/ลบรายชื่อ")
    with st.form("add_form"):
        aid = st.text_input("รหัสนักเรียน")
        aname = st.text_input("ชื่อ-นามสกุล")
        if st.form_submit_button("เพิ่มชื่อ"):
            if aid and aname:
                st.session_state.students.append({"id": aid, "name": aname, "score": 100})
                st.session_state.temp_status[aid] = "ยังไม่เช็ค" # เพิ่มรหัสในสถานะชั่วคราวด้วย
                st.rerun()
    ...
    if len(st.session_state.students) > 0:
        ...
        if st.button("ลบชื่อนักเรียน"):
            tid = to_del.split(" - ")[0]
            st.session_state.students = [s for s in st.session_state.students if s['id'] != tid]
            st.rerun()